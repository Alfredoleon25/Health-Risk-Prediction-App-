...
# MPS doesn't support autocast, so use nullcontext for both CPU and MPS
...
# MPS doesn't support GradScaler, so use 'cpu' as backend when using MPS
scaler_backend = 'cuda' if device_type == 'cuda' else 'cpu'
scaler = torch.amp.GradScaler(scaler_backend, enabled=(dtype == 'float16')) 
